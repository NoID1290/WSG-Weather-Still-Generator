...
namespace ECCC
{
    ...
    public static class ECCCApi
    {
        ...
        /// <summary>
        /// Fetches real-time or historical lightning flash data from ECCC for a
        ...
        public static async Task<System.Collections.Generic.List<Models.LightningFlash>> GetLightningStrikesAsync(
            HttpClient httpClient,
            ...
            int limit = 5000)
        {
            try
            {
                LogMessage($"[ECCC API] Fetching lightning: {from:u} → {to:u}");
                var svc = new LightningService(httpClient);
                var flashes = await svc.FetchLightningStrikesAsync(bbox, from, to, limit);
                LogMessage($"[ECCC API] ✓ {flashes.Count} lightning flashes received");
                return flashes;
            }
            catch (Exception ex)
            {
                LogMessage($"[ECCC API] Error fetching lightning: {ex.Message}");
                return new System.Collections.Generic.List<Models.LightningFlash>();
            }
        }

        /// <summary>
        /// Returns available UTC timestamps for the ECCC lightning density WMS layer.
        /// Useful when syncing the lightning overlay to the radar animation timeline.
        /// </summary>
        /// <param name="httpClient">HttpClient instance</param>
        /// <param name="numFrames">Maximum number of timestamps to return</param>
        public static async Task<System.Collections.Generic.List<DateTime>> GetLightningTimestampsAsync(
            HttpClient httpClient,
            int numFrames = 12)
        {
            try
            {
                var svc = new LightningService(httpClient);
                return await svc.FetchLightningTimestampsAsync(numFrames);
            }
            catch (Exception ex)
            {
                LogMessage($"[ECCC API] Error fetching lightning timestamps: {ex.Message}");
